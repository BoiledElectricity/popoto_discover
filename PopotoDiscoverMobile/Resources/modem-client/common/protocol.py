...
def validate_target_selector(message: Dict[str, Any]) -> None:
    """
    Validate that a request selects a device by CPU UID/device ID.

    MAC addresses are intentionally not valid command selectors. PMM Ethernet
    MACs may be generated and are not guaranteed to be globally unique.
    """
    target_serial = str(message.get('target_serial') or message.get('target_id') or '').strip()

    if not target_serial:
        raise ValidationError("Missing target selector: target_serial or target_id")
...
def _add_target_selector(message: Dict[str, Any], target_id: Optional[str] = None,
                         target_serial: Optional[str] = None) -> Dict[str, Any]:
    selected_target = target_serial or target_id
    if selected_target:
        message['target_serial'] = selected_target
        message['target_id'] = selected_target
    validate_target_selector(message)
    return message


def create_set_ip_message(nonce: str, target_id: Optional[str], new_ip: str,
    ...
    Args:
        nonce: Unique nonce for this request
        target_id: CPU UID/device ID of target device
    ...
    message = _add_target_selector(message, target_id, target_serial)
...
def create_set_rtc_message(nonce: str, target_id: Optional[str], rtc: str,
                           secret: Optional[str] = None,
                           target_serial: Optional[str] = None) -> Dict[str, Any]:
    """
...
def create_get_rtc_message(nonce: str, target_id: Optional[str],
                           secret: Optional[str] = None,
                           target_serial: Optional[str] = None) -> Dict[str, Any]:
    """
...
def create_set_param_message(nonce: str, target_id: Optional[str], param_name: str,
...
def create_set_uboot_env_message(nonce: str, target_id: Optional[str], name: str,
...
def create_reboot_message(nonce: str, target_id: Optional[str],
                          secret: Optional[str] = None,
                          target_serial: Optional[str] = None) -> Dict[str, Any]:
    message = {
        'cmd': MSG_REBOOT,
...
def create_shell_exec_message(nonce: str, target_id: Optional[str], command: str,
...
def create_get_version_message(nonce: str, target_id: Optional[str],
                                secret: Optional[str] = None,
                                target_serial: Optional[str] = None) -> Dict[str, Any]:
    """
