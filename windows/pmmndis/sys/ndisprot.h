...
#define NT_DEVICE_NAME          L"\\Device\\PmmNdis"
#define DOS_DEVICE_NAME         L"\\Global??\\PmmNdis"
...
typedef struct _NDISPROT_GLOBALS
{
    ...
    USHORT                  EthType;            // legacy sample field; PMM accepts a fixed protocol set
...
//
// The Ethernet header is read as a little-endian USHORT. These constants map to
// the wire EtherTypes used by PMM host tooling:
//   0x88A2 ATA over Ethernet
//   0x88B5 PMM U-Boot Ethernet console
//   0x88B6 PMM discovery L2 JSON
//
#define PMM_ETH_TYPE_AOE             0xa288
#define PMM_ETH_TYPE_CONSOLE         0xb588
#define PMM_ETH_TYPE_DISCOVERY       0xb688

__inline BOOLEAN
PmmNdisIsAcceptedEthType(
    _In_ USHORT EthType
    )
{
    return EthType == PMM_ETH_TYPE_AOE ||
           EthType == PMM_ETH_TYPE_CONSOLE ||
           EthType == PMM_ETH_TYPE_DISCOVERY;
}
